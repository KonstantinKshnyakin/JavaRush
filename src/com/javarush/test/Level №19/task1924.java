    ...
    public static void main(String[] args) throws IOException {
        ...
        while ((inputStr = bufferedReaderFile.readLine()) != null) {
            ...
            for (int i = 0; i < splitInputStr.length; i++) {
                int countPrint = 0;
                for (Map.Entry<Integer, String> entryMapNumber : map.entrySet()) {
                    if (splitInputStr[i].equals(String.valueOf(entryMapNumber.getKey()))) {
                        splitInputStr[i] = entryMapNumber.getValue();
                        System.out.print(splitInputStr[i] + " ");
                        break;
                    }
                    countPrint++;
                }
