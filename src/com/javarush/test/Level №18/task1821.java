...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        SortedMap<Integer, Integer> quantityChar = new TreeMap<>();

        while (fileInputStream.available() > 0) {
            int nextByte = fileInputStream.read();
            
            if (quantityChar.containsKey(nextByte)) {
                quantityChar.put(nextByte, quantityChar.get(nextByte) + 1);
            } else {
                quantityChar.put(nextByte, 1);
            }
        }

        for (Map.Entry<Integer, Integer> entryQuantityChar : quantityChar.entrySet()) {
            System.out.println((char) (int) entryQuantityChar.getKey() + " " + entryQuantityChar.getValue());
        }
