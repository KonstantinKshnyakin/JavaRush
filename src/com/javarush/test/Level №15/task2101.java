...
public class Solution {
    ...
    public static byte[] getNetAddress(byte[] ip, byte[] mask) {
        byte[] netAddress = new byte[]{(byte)(ip[0] & mask[0]), (byte)(ip[1] & mask[1]), (byte)(ip[2] & mask[2]), (byte)(ip[3] & mask[3]) };
        return netAddress;
    }

    public static void print(byte[] bytes) {
        for (byte b: bytes) {
            if (Integer.toBinaryString(b).length() < 9) {
...
//            System.out.printf("%s %s %8s\n\r", Integer.toBinaryString(bytes[0]).substring(24 ,32),
//                                            Integer.toBinaryString(bytes[1]).substring(24 ,32),
//                                            Integer.toBinaryString(bytes[2]));
